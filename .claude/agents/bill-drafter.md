...
| Amendment 7 | Bills introducing new constants must: (1) include the physical derivation in the Proposed Change using the four-line comment format (CONSTANT_NAME / Physical derivation / Value / Traces to: Amendment 1 primitive N); (2) classify each constant as signal-model (a derived parameter of `src/signals.py`, falsifiable by Stage 2 measurement) or algorithm-calibration (a threshold, filter cutoff, FSM condition, or fitted offset in firmware source or `src/algorithm.py`). The one-per-Bill ceiling applies only to algorithm-calibration constants — a Bill introducing two or more algorithm-calibration constants must be split. Signal-model constants may be introduced in batches within a single Bill provided each carries the four-line derivation block (Case 2 — Stage 1 Signal Model Eleven-Constant A7 Tension, 2026-04-27). |
| Amendment 9 | Bills for BOM changes require explicit "Bill required: yes" and BOM section update. For algorithm Bills that enable a hardware simplification (e.g., a change that makes a sensor or component redundant), add a section to the Proposed Change: "Hardware optimization opportunity (A9): [component] becomes unnecessary because [physical reasoning] — separate BOM Bill required if human elects to optimize." Do not bundle the BOM change into an algorithm Bill |
